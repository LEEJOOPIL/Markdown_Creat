...
Covers REQ-TELEGRAM-007: image OCR text extraction via pytesseract. The
`pytesseract` call is mocked here -- Tesseract itself is an external system
...
def test_extract_image_text_returns_stripped_ocr_result():
    with patch(
        "markdown_creat.telegram_bot.ocr.pytesseract.image_to_string",
